...
**- Justification:** This environment variable override breaks the CI pipeline because the project relies on specific versions of GitHub actions (like `actions/checkout` v3.1.0) that are expected to run in their native environments. Forcing Node 24 causes unintended side effects.
...
**- Pattern:** Creating an `ErrorReporter.java` or similar centralized error-handling utility and retroactively replacing functional `Log.e` or exception-handling blocks across the codebase.
**- Justification:** Despite generic agent instructions suggesting centralized error reporting, these sweeping refactoring PRs are consistently rejected in this repository as they introduce unnecessary churn, complicate simple logging, and often break existing error flows.
...
**- Pattern:** PRs where an agent (e.g., Docs, Denoiser) modifies files outside its strictly defined persona scope (e.g., a Docs agent modifying `mise.toml`, or Denoiser modifying `.github/workflows/autorelease.yml`).
